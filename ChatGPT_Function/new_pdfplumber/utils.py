...
def cluster_objects(objs, attr, tolerance): # attr: 'doctop'
    if isinstance(attr, (str, int)):
        attr_getter = itemgetter(attr)
    else:
        attr_getter = attr
    objs = to_list(objs) # 한 글자에 대한 dict들을 모아 놓은 list.
...
def collate_line(line_chars, tolerance=DEFAULT_X_TOLERANCE, layout=False) -> str: # line_chars: 페이지 구성의 한 line. (순서대로 하나씩 들어옴)
    coll = ""
    last_x1 = None
    for char in sorted(line_chars, key=itemgetter("x0")):
        if (last_x1 is not None) and (char["x0"] > (last_x1 + tolerance)):
            coll += " " # 테이블 한 row의 텍스트들에 대해, column 기준으로 텍스트 구분하는 로직. (띄어쓰기로 구분)
        last_x1 = char["x1"]
        coll += char["text"]
    return coll
...
def extract_text(
    ...
    **kwargs,
    ...
    josa = ('이', '가', '께서', '에서', '어서', '에게서', '의', '을', '를', '에', '에게', '께', '한테', '로서', '로써', '고', '라고', '와', '과', '랑', '같이', '처럼', '만큼', '만치','하고', '이며', '에다', '에다가', '랑', '은', '는', '도', '부터', '까지', '마저', '조차', '만', '따라', '따라서', '토록', '치고', '대로', '나', '란', '든가', '든지', '나마', '로', '으로', '는데', '며', '되어', '위해', '대해', '한', '하지', '해', '하여', '인', '된', '각각', '음', '함', '아', '하기', '될', '할', '매우')
    roma_num = ('I.', 'II.', 'III.', 'IV.', 'V.', 'VI.', 'VII.', 'VIII.', 'IX.', 'X.', 'XI.', 'XII.')
    round_num = ('①', '②', '③', '④', '⑤', '⑥', '⑦', '⑧', '⑨', '⑩', '⑪', '⑫', '⑬', '⑭', '⑮')
    context_token = ('-', ':', '=', '/')
    ...
    if layout:
        ...
        if text_flag and tables: # 본문 text 추출에 사용되는 경우(extract_tables 과정에 필요한 부분 X)
            chars = sorted(chars, key=lambda x: x['doctop'])
            for table in tables:
                table_start_ps = table.bbox[1]
                table_end_ps = table.bbox[3]
                start_flag = False
                end_flag = False
                for char_idx, char in enumerate(chars):
                    if not start_flag and char['top'] >= table_start_ps: # 본문 텍스트 중 table 시작 위치
                        char['text'] = '[TABLE_START]' + char['text'] # table 시작 텍스트 바로 앞에 시작 문구 추가.
                        start_flag = True
                    elif not end_flag and char['bottom'] > table_end_ps: # 본문 텍스트 중 table 끝 위치.
                        chars[char_idx - 1]['text'] += '[TABLE_END]'
                        end_flag = True
                    elif start_flag and not end_flag and char_idx == len(chars) - 1: # chars의 마지막 텍스트가 table 끝 위치인 경우
                        chars[char_idx]['text'] += '[TABLE_END]'
                        end_flag = True
                    elif start_flag and end_flag: # table 영역 처리 완료.
                        break
        ...
        if not text_flag:
            lines = list(lines) # 제네레이터를 미리 리스트로 변환하고 사용해야 함.
            for line_idx, now_line in enumerate(lines):
                now_line = now_line.strip()

                if line_idx == 0: # 첫 번째 line.
                    concat_text += re.sub(f'^\d\.|^\d\)|^\(\d+\)|^[가-하]\.|^{str(list(roma_num))}|^{str(list(round_num))}', '', now_line)
                else:
                    past_line = lines[line_idx - 1].strip()

                    if past_line.endswith(('.', ',', '%')) or now_line.startswith(context_token) or now_line.startswith(('및', '그')):
                        concat_text += ' ' + re.sub(f'^\d\.|^\d\)|^\(\d+\)|^[가-하]\.|^{str(list(roma_num))}|^{str(list(round_num))}', '', now_line)
                    elif re.match('.*[^\d]$', past_line) and re.match('^\d.*', now_line):
                        concat_text += ' ' + re.sub(f'^\d\.|^\d\)|^\(\d+\)|^[가-하]\.|^{str(list(roma_num))}|^{str(list(round_num))}', '', now_line)
                    elif past_line.endswith(josa):
                        if past_line.endswith('으로') and now_line.startswith(('서', '써', '는')):
                            concat_text += now_line
                        elif past_line.endswith('가') and now_line.startswith(('액', '설', '능', '정', '된')):
                            concat_text += now_line
                        elif past_line.endswith('함') and now_line.startswith('으로'):
                            concat_text += now_line
                        elif past_line.endswith('도') and now_line.startswith('록'):
                            concat_text += now_line
                        elif past_line.endswith('에') and now_line.startswith(('게', '서')):
                            concat_text += now_line
                        elif past_line.endswith('해') and now_line.startswith('당'):
                            concat_text += now_line
                        elif past_line.endswith('만') and now_line.startswith('져'):
                            concat_text += now_line
                        elif past_line.endswith('이') and now_line.startswith('하'):
                            concat_text += now_line
                        elif past_line.endswith('인') and now_line.startswith('감'):
                            concat_text += now_line
                        else:
                            concat_text += ' ' + re.sub(f'^\d\.|^\d\)|^\(\d+\)|^[가-하]\.|^{str(list(roma_num))}|^{str(list(round_num))}', '', now_line)
                    elif past_line.endswith(('.', ',', '및')) or re.match('^\d\.|^\d\)|^\(\d+\)|^[가-하]\.|^주\d\)', now_line) or now_line.startswith(roma_num) or now_line.startswith(round_num) or now_line.startswith('-') or re.match('.*[십,백,천,만]원$', past_line):
                        concat_text += ' ' + re.sub(f'^\d\.|^\d\)|^\(\d+\)|^[가-하]\.|^{str(list(roma_num))}|^{str(list(round_num))}', '', now_line)
                    else:
                        concat_text += re.sub(f'^\d\.|^\d\)|^\(\d+\)|^[가-하]\.|^{str(list(roma_num))}|^{str(list(round_num))}', '', now_line)
            return concat_text
