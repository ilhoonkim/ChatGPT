...
class Page(Container):
    ...
    def process_object(self, obj): # obj: pdfminer의 LTChar 객체. matrix, font, adv, text 속성 지님.
    ...
    def extract_tables(self, table_settings={}):
        ...
        self.tables = self.find_tables(table_settings) # self.tables 안의 한 table: table 자체의 bbox 범위와 cells들의 범위 값을 가지고 있음.
        ...
        return [table.extract(**extract_kwargs) for table in self.tables]
    ...
    def extract_text(self, **kwargs):
        return utils.extract_text(
            chars=self.chars, tables=self.tables, text_flag=True, x_shift=self.bbox[0], y_shift=self.bbox[1], **kwargs
        )
