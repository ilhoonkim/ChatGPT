...
class Table(object):
    def __init__(self, page, cells):
        ...
        self.past_xdict = None

    @property
    def rows(self):
        _sorted = sorted(self.cells, key=itemgetter(1, 0)) # 한 테이블 별, 셀 당 '(x시작, y시작, x끝, y끝)' 좌표 튜플을 요소로 지닌 리스트.
        self.xs = list(map(str, list(sorted(set(map(itemgetter(0), self.cells)))))) # 한 테이블 별, 가장 세분화된 세로선(x위치) 시작 위치를 요소로 지닌 리스트.
        rows = []
        for y, row_cells in itertools.groupby(_sorted, itemgetter(1)):
            now_xdict = dict((str(cell[0]), cell) for cell in row_cells) # 테이블의 한 row 별, 셀 당 세로선(x위치) 시작 위치를 key값으로 지닌 dict.
            last_x1 = list(now_xdict.values())[-1][2]
            if not self.past_xdict: # 첫 번째 row.
                for x_idx, x in enumerate(self.xs):
                    if x not in now_xdict: # col merged.
                        temp_list = list(now_xdict[self.xs[x_idx - 1]]) # col merged 이므로, 직전 셀의 value 가져오기.
                        temp_list[0] = float(x)
                        try:
                            temp_list[2] = float(self.xs[x_idx + 1]) # 다음 셀의 x0 값.
                        except IndexError: # 마지막 셀
                            temp_list[2] = last_x1 # 테이블의 가장 바깥 x1 값.

                        if '[COL_MERGED]' not in temp_list:
                            temp_list.append('[COL_MERGED]') # 튜플 안의 다섯 번째 값으로 col_merged 토큰 삽입.
                        
                        now_xdict[x] = tuple(temp_list)
            else: # 두 번째 row 부터.
                row_min_y1 = min([now_tuple[3] for now_tuple in now_xdict.values()]) # 현재 row의 가장 작은 y1 값.
                for cell_idx, past_cell_x0 in enumerate(self.past_xdict.keys()): # past_key: 이전 row 셀들의 x0값. (모든 x0값)
                    if past_cell_x0 not in now_xdict:
                        ## row merged 인지 col merged 인지 구분하는 로직. (제일 중요!!!!!!)                       
                        if self.past_xdict[past_cell_x0][3] >= row_min_y1: # row merged.
                            temp_list = list(self.past_xdict[past_cell_x0])
                            if '[ROW_MERGED]' not in temp_list:
                                temp_list.append('[ROW_MERGED]')
                        else: # col merged.
                            temp_list = list(now_xdict[list(self.past_xdict.keys())[cell_idx - 1]])
                            temp_list[0] = float(past_cell_x0)
                            try:
                                temp_list[2] = float(list(self.past_xdict.keys())[cell_idx + 1]) # 다음 셀의 x0 값.
                            except IndexError: # 마지막 셀
                                temp_list[2] = last_x1 # 테이블의 가장 바깥 x1 값.

                            if '[COL_MERGED]' not in temp_list:
                                temp_list.append('[COL_MERGED]')
                        
                        now_xdict[past_cell_x0] = tuple(temp_list)
                    
            sorted_xtuple = sorted(now_xdict.items(), key=lambda x: int(x[0].split('.')[0]))
            now_xdict = dict((x, y) for x, y in sorted_xtuple) # 세로선(x위치) 기준 딕셔너리 오름차순 정렬.

            row = Row(list(now_xdict.values())) # 이미 첫 번째 row의 now_xdict에 테이블 모든 세부 x0에 대한 값이 들어감.
            rows.append(row)

            self.past_xdict = now_xdict
        return rows
    ...
    def extract(
        ...
        def char_in_bbox(char, bbox):
            ...
            try:
                x0, top, x1, bottom = bbox
            except ValueError: # row merged 토큰이 튜플에 추가돼 있어서, 튜플 unpack 시 사이즈가 다른 경우
                x0, top, x1, bottom, row_merged_token = bbox
            return (
                (h_mid >= x0) and (h_mid < x1) and (v_mid >= top) and (v_mid < bottom)
            )

        for row in self.rows: # row: 각 table들의 한 row. 'cells', 'bbox' 속성 지님.
            arr = []
            row_chars = [char for char in chars if char_in_bbox(char, row.bbox)]

            for cell in row.cells:
                if '[COL_MERGED]' in cell:
                    cell_text = '[COL_MERGED]'
                elif '[ROW_MERGED]' in cell:
                    cell_text = '[ROW_MERGED]'
                else:
                    cell_chars = [
                        char for char in row_chars if char_in_bbox(char, cell)
                    ]

                    if len(cell_chars): # table 텍스트 부분 추출하는 부분. (table 구조에 맞게 추출됨.)
